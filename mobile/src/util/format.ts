export const formatNumToWan = (val?: number) => {
  ...
  if (val >= 10000) {
    ...
    return `${turnVal[0]}${hasDecimal?'.':''}${ hasDecimal ? turnVal[1][0] : ''}W`;
  }
  return val.toString();
};
...
export const numberToChinese = (inputNum?: number | string): string => {
  ...
  const chineseNumbers: string[] = ['', '一', '二', '三', '四', '五', '六', '七', '八', '九'];
  ...
  while (num > 0) {
    const digit = num % 10;
    if (digit > 0) {
      if (unitIndex === 1 && digit === 1) result = unitNames[unitIndex] + result;
