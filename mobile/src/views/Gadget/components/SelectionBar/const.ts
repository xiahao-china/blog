export class SelectionBar {
  ...
  checkBlockInfo() {
    ...
    this.itemMargin = (this.listEl.getBoundingClientRect().width - (this.itemWidth * len)) / (len - 1);
    this.currentIndexX = (this.itemWidth + this.itemMargin) * this.currentIndex * -1;
  }

  onTouchStart(event: TouchEvent) {
    event.preventDefault();
    this.checkBlockInfo();
    const clickIntervalX = this.listEl.getBoundingClientRect().x - (event.target as Element).getBoundingClientRect().x;
    this.startTouchIndex = Math.abs(clickIntervalX) > this.itemMargin ?
      Math.floor(Math.abs(clickIntervalX) / (this.itemWidth + this.itemMargin)) : 0
  ...
  onTouchMove(event: TouchEvent) {
    ...
    requestAnimationFrame(() => {
      const calcVal =event.targetTouches[0].screenX - this.startX + this.currentIndexX;
      this.currentX = calcVal > 0 ? 0 : calcVal;
      this.listEl.setAttribute('style', `transform: translateX(${this.currentX}px);`);
    })
  }

  public changeIndex = (index: number, aimX?: number) => {
    if(this.currentIndex !== index) {
      this.currentX = (this.itemWidth + this.itemMargin) * this.currentIndex * -1;
  ...
  onTouchEnd() {
    ...
    this.hasStart = false;
    this.currentIndex = Math.floor(Math.abs(this.currentX) / (this.itemWidth + this.itemMargin)) +
      Number((Math.abs(this.currentX) % this.itemWidth) > this.itemWidth * this.decisionRatio);
    if (this.currentIndex < 0) this.currentIndex = 0;
    if (this.currentIndex > this.childrenMaxIndex) this.currentIndex = this.childrenMaxIndex;
    this.currentIndexX = (this.itemWidth + this.itemMargin) * -1 * this.currentIndex;
    this.listEl.setAttribute('style', `transform: translateX(${this.currentIndexX}px);`);
