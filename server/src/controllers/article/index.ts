...
export const createAndEditArticleControllers = async (
  ...
  const { id, title, content, isHTML, isPrivate, cover } =
    ctx.request.body || {}
  if (!title && !content)
    return sendResponse.error(ctx, '文章标题或内容不能为空!')
  if (cover && !WHITELIST_HOST.find((item) => cover.includes(item)))
    return sendResponse.error(ctx, '封面域名非法!')
  try {
    const nowArticle: IArticle = id
      ? await articleModel.collection.findOne({ id })
      : undefined
    if (nowArticle) {
      if (nowArticle.createrUid !== userInfo.uid)
...
export const getArticleDetailControllers = async (
  ctx: TDefaultRouter<{ id: string }>,
  next: TNext
) => {
  ...
  const createrUserInfo: IUserInfo = await userModel.collection.findOne({
    uid: article.createrUid
  })
  try {
    articleModel.collection.updateOne(
      { id },
      { $set: { browseNum: article.browseNum + 1 } },
      {}
    )
  } catch (err) {
    console.log(err)
  ...
  sendResponse.success(ctx, {
    ...
    hasFollow: (userInfo ? userInfo.followUid : []).includes(
      createrUserInfo.uid
    )
  })
}
...
export const articleListControllers = async (
  ...
  if (!pageSize || !pageNumber)
    return sendResponse.error(ctx, '传参缺失，请检查pageSize与pageNumber!')
  try {
    const filterObj = getArticleListControllersFilterObj(
      (userInfo as IUserInfo)?.uid || ''
    )
    ...
    userInfoList
      .filter((item) => item)
      .forEach((item) => (userInfoMap[item.uid] = item))
...
export const deleteArticleControllers = async (
  ctx: TDefaultRouter<{ id: string }>,
  next: TNext
) => {
  ...
  try {
    const article: IArticle = await articleModel.collection.findOne({ id })
    if (!article) return sendResponse.error(ctx, '文章不存在')
    if (article.createrUid !== userInfo.uid)
      return sendResponse.error(ctx, '权限不足')
...
export const likeArticleControllers = async (
  ctx: TDefaultRouter<{ id: string }>,
  next: TNext
) => {
  const { id } = ctx.request.body || {}
...
export const collectArticleControllers = async (
  ctx: TDefaultRouter<{ id: string }>,
  next: TNext
) => {
  const { id } = ctx.request.body || {}
...
export const saveDraftArticleControllers = async (
  ctx: TDefaultRouter<ISaveDraftReqParams>,
  next: TNext
) => {
  const userInfo = await checkLogin(ctx, next)
  if (!userInfo) return sendResponse.error(ctx, '', EReqStatus.noLogin)
  ...
  if (!title && !content)
    return sendResponse.error(ctx, '文章标题或内容不能为空!')
  try {
    const nowDraftArticle: IDraftArticle =
      await draftArticleModel.collection.findOne({ creatorUid: userInfo.uid })
    if (nowDraftArticle) {
...
export const delDraftArticleControllers = async (
  ctx: TDefaultRouter<{}>,
  next: TNext
) => {
  const userInfo = await checkLogin(ctx, next)
  ...
  try {
    const nowDraftArticle: IDraftArticle =
      await draftArticleModel.collection.findOne({ creatorUid: userInfo.uid })
    if (nowDraftArticle) {
      if (nowDraftArticle.creatorUid !== userInfo.uid)
        return sendResponse.error(ctx, '权限不足')
...
export const getDraftArticleControllers = async (
  ctx: TDefaultRouter<{}>,
  next: TNext
) => {
  const userInfo = await checkLogin(ctx, next)
  if (!userInfo) return sendResponse.error(ctx, '', EReqStatus.noLogin)
  try {
    const creatorUserInfo: IUserInfo =
      await draftArticleModel.collection.findOne({ creatorUid: userInfo.uid })
